...
-- Create RLS policies
-- Allow public read access to videos, assets, download_log, site_config and comments
...
-- Allow admin full access to videos, assets, download_log, site_config
...
-- Allow public insert access to comments (for anonymous users to post comments)
CREATE POLICY "Allow public insert access" ON comments FOR INSERT WITH CHECK (true);

-- Allow admin full access to comments
CREATE POLICY "Allow full access for authenticated users" ON comments FOR ALL USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated');
